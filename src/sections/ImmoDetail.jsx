...
const ImmoDetail = () => {
  ...
  return (
      <div ref={immoDetailRef} className="detail-container">
        ...
        {/* Slider */}
        <div className="flex flex-row justify-centeralign-center relative">
          <button className="text-4xl mr-5" onClick={() => handleImageChange((currentImageIndex - 1 + immo.imgSlider.length) % immo.imgSlider.length)} aria-label="Vorheriges Bild" >
            &lt;
          </button>

          <img src={immo.imgSlider[currentImageIndex]} alt={`immo-${immo.id}`} className="rounded-xl my-5 object-cover w-[300px] h-[200px]  md:w-[600px] md:h-[400px]" />

          <button className="text-4xl ml-5" onClick={() => handleImageChange((currentImageIndex + 1) % immo.imgSlider.length)} aria-label="Nächstes Bild" >
            &gt;
          </button>
        </div>
